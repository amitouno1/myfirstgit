...
//        order.setPriceSet(priceSet);
        ...
        {
	        //加入priceset
	        List _priceSetList=goods.getPriceSetObjectList();
	        String priceTitle= getPara("priceTitle");
	        List<Map> priceSetList=new ArrayList();
	        for(int i=0;i<_priceSetList.size();i++)
	        {
	        	
	        	String _title=((Map)_priceSetList.get(i)).get("title").toString();
	        	String _price=((Map)_priceSetList.get(i)).get("price").toString();
	        	String _number="";
	        	if(priceTitle.equals(_title))
	        	{
	        		_number=number.toString();
	        		((Map)_priceSetList.get(i)).put("number", _number);
	        	}else
	        	{
//	        		_number=((Map)_priceSetList.get(i)).get("number").toString();
	        		((Map)_priceSetList.get(i)).put("number", "0");
	        		
	        	}
	        	priceSetList.add(((Map)_priceSetList.get(i)));
	        }
	        order.setPriceSet(new Gson().toJson(priceSetList));
        }else
        {
        	//加入priceset
	        List _priceSetList=goods.getPriceSetObjectList();
//	        String priceTitle= getPara("priceTitle");
	        ...
	        for(int i=0;i<_priceSetList.size();i++)
	        {
	        	...
	        	if("儿童".equals(_title))
	        	{
	        		_number=number2.toString();
	        		((Map)_priceSetList.get(i)).put("number", _number);
	        	}else
	        	{
	        		_number=number1.toString();
//	        		_number=((Map)_priceSetList.get(i)).get("number").toString();
	        		((Map)_priceSetList.get(i)).put("number", _number);
	        	}
	        	priceSetList.add(((Map)_priceSetList.get(i)));
...
//                GoodsDao.dao.save(goods);
    ...
    public void updateCart() {
        ...
        for (Order o : ol) {
            ...
            {
            	oo = upOrderLine(oo, travelers, o.getNumber(), getPara("contactName"), getPara("contactPhone"), getPara("message"), getParaToInt("payMethod"));
            }
...
//    }
    ...
    public void updateLine() {
        ...
        order = upOrderLine(order, travelers, getParaToInt("number", 0), getPara("contactName"), getPara("contactPhone"), getPara("message"), getParaToInt("payMethod"));
...
//        order.setPrice(price.intValue());
    ...
    private Order upOrderLine(Order order, List<Traveler> travelers, Integer number, String contactName, String contactPhone, String message, Integer payMethod) {
    	...
    	List priceSetList=order.getPriceSetObjectList();

        Double price = 0.0;
        if (order.getGoodsObject().getPriceType().equals(1)) {
        	
        	Double chengrenPrice=(Double) ((Map)priceSetList.get(1)).get("price");
        	Double ertongPrice=(Double) ((Map)priceSetList.get(0)).get("price");
        	Integer chengrenNum=order.getNumber1();
        	Integer ertongNum=order.getNumber2();

            price = (chengrenPrice * chengrenNum)+ertongPrice*ertongNum;
