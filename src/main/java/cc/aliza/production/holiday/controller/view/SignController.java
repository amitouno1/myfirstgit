...
public class SignController extends Controller {
    ...
    @Before(POST.class)
    public void doSign() {
        String username = getPara("username").toString().trim();
        String password = getPara("password").toString().trim();
        String redirect = getPara("redirect", "/").toString().trim();
        String password2 = getPara("password2").toString().trim();
        try {
            ...
            if (StringUtils.isBlank(username)||"邮箱/用户名/手机号".equals(username)) {
