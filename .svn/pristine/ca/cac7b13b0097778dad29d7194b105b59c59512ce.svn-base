...
public class GoodsController extends Controller {
    ...
    public void pj() {
        ...
        try{
        	 String username_search = getPara("username_search", null);
        ...
        Page<PJ> pjList=pjDao.dao.findBy(params);
        
        setAttr("pjList", pjList);
        
        render("/manage/goods/pj/index.html");
    }
    
    //@Before(POST.class)
    public void savepj() {
    	String id = getPara("id", null);
    	String fwzl=getPara("fwzl", null);
    	String xcap=getPara("xcap", null);
    	String sspz=getPara("sspz", null);
    	Integer ztpj=Integer.parseInt(fwzl) +Integer.parseInt(xcap) +Integer.parseInt(sspz);
    	
    	pjDao.dao.set(id, "fwzl", fwzl);
    	pjDao.dao.set(id, "xcap", xcap);
    	pjDao.dao.set(id, "sspz", sspz);
    	pjDao.dao.set(id, "ztpj", ztpj.toString());
    ...
    public void replyComment() {
    	...
    	String CommentID = getPara("id");
    	String replyContent=getPara("replyContent");
    	
    	Comment comment = CommentDao.dao.findOne(CommentID);
    	
    	Date now = new Date(); 
        String replyDate=now.toLocaleString();

    	comment.setReplyDate(replyDate);
    ...
    public void save() {
        ...
        String[] lineTSPics =new String[]{};
        List<Image> lineTSPicsList = new ArrayList<Image>();

        if(getParaValues("lineTSPics")!=null)
        {
        	lineTSPics = getParaValues("lineTSPics");
        	 try{
        ...
        goods.setCollect(collect);
        
        Integer sales=getParaToInt("sales");
