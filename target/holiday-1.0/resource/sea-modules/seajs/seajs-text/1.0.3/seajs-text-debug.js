...
define("seajs/seajs-text/1.0.3/seajs-text-debug", [], function () {
    ...
    register({
        ...
        ext: [ ".juicer" ],
        exec: function (uri, content) {
            var code = [ 'define("' + uri + '#", ["juicer"], function(require, exports, module) {', '  var source = "' + jsEscape(content) + '"', '  var juicer = require("juicer")', "  module.exports = function(data) {    return juicer(source).render(data)", "  }", "})" ].join("\n");
            globalEval(code);
        }
    });
